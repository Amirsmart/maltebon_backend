...
from db_models.plugins import PluginCrudModel, get_one_plugin, get_one_plugin_crud, init_plugins, set_plugin_token
...
class TestApp(unittest.TestCase):
    ...
    def test4_getsketch(self):
        user = get_by_username(names[0] ,engine)
        self.assertIsNotNone(user)
        for i in range(len(emails)):
            name = names[i]
            self.assertIsNotNone(get_user_sketch_by_name(user.id , name , engine))
        
        

        
    def test3_addsketch(self):
        user = get_by_username(names[0] ,engine)
        self.assertIsNotNone(user)
        for i in range(len(emails)):
            data=emails[i]
            name = names[i]
            self.assertIsNotNone(add_sketch(user,engine,data,name))
